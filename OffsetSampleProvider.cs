...
namespace SpanTest
{
    ...
    public class OffsetSampleProvider : ISampleProvider
    {
        ...
        public int Read(Span<float> buffer)
        {
            ...
            if (phase == 1) // delay
            {
                int delaySamples = Math.Min(buffer.Length, DelayBySamples - phasePos);
            ...
            if (phase == 3) // take
            {
                int samplesRequired = buffer.Length - samplesRead;
                if (takeSamples != 0)
            ...
            if (phase == 4) // lead out
            {
                int samplesRequired = Math.Min(buffer.Length - samplesRead, LeadOutSamples - phasePos);
